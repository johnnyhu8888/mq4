        ...
        for (cnt=0;cnt<OrdersTotal();cnt++)
          {
             OrderSelect(cnt,SELECT_BY_TICKET,MODE_TRADES);
              ...
              if (OrderType()==OP_SELL && close_05_buffer[1]-OrderOpenPrice()<=-0.0010) 
                
                 OrderModify(OrderTicket(),OrderOpenPrice()-4*Point,OrderOpenPrice()-4*Point,OrderTakeProfit(),0,Blue);
             
              if (OrderType()==OP_BUY && close_05_buffer[1]-OrderOpenPrice()>=0.0010) 
