...
//*** Buy  1  ***************************************************************************************** 
 ...
 //*** Buy  3  ***************************************************************************************** 
       ...
       if (macdsignalline_15_buffer[1]-macdsignalline_15_buffer[2]>=-0.00005 && 
           macdsignalline_15_buffer[1]-macdsignalline_15_buffer[2]<=0 &&
           macdsignalline_15_buffer[2]-macdsignalline_15_buffer[3]>=-0.00010)
           if (ma45_15_buffer[1]-ma45_15_buffer[6]<=-0.0006)
               return;
               else
       
           OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-18*Point,Ask+100*Point,"My order #B3",8803,0,Blue );
 ...
 //*** sell  3  ***************************************************************************************** 
       ...
       //    macdmainline_15_buffer[1]-macdsignalline_15_buffer[1]>=-0.00010) 
         if (ma45_15_buffer[1]-ma45_15_buffer[6]>=0.0006)
            ...
            OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+18*Point,Bid-100*Point,"My order #S3",4403,0,Lime );
     } 
  //***  Ordermodify   ********************************************************************************** 
        ...
        for (cnt=0;cnt<OrdersTotal();cnt++)
          {
             OrderSelect(cnt,SELECT_BY_POS,MODE_TRADES);
              ...
              if (OrderType()==OP_BUY && high_15_buffer[0]-OrderOpenPrice()>=0.0070 &&
                  close_15_buffer[1]-open_15_buffer[1]>=0.0001) 
                  OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()+30*Point,OrderTakeProfit(),0,Blue);
             
              if (OrderType()==OP_BUY && high_15_buffer[0]-OrderOpenPrice()>=0.0040 && high_15_buffer[0]-OrderOpenPrice()<=0.0045 &&
                  close_15_buffer[1]-open_15_buffer[1]>=0.0001) 
              ...
              if (OrderType()==OP_SELL && low_15_buffer[0]-OrderOpenPrice()<=-0.0070 &&
                  close_15_buffer[1]-open_15_buffer[1]<=-0.0001) 
                  OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()-30*Point,OrderTakeProfit(),0,Blue);
             
              if (OrderType()==OP_SELL && low_15_buffer[0]-OrderOpenPrice()<=-0.0040 && low_15_buffer[0]-OrderOpenPrice()>=-0.0045 &&
                  close_15_buffer[1]-open_15_buffer[1]<=-0.0001) 
                  OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()-4*Point,OrderTakeProfit(),0,Blue);
