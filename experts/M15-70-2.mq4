...
//***  Ordermodify   ********************************************************************************** 
  ...
  //          OrderModify(OrderTicket(),OrderOpenPrice(),M15highest16bar-50*Point,OrderTakeProfit(),0,Blue);
     ...
     if  (TimeCurrent()-OrderOpenTime()>=5000 &&  OrderType()==OP_BUY && (shortbuy01==OrderTicket() || shortbuy02==OrderTicket() || 
      ...
      if (OrderType()==OP_BUY && (shortbuy01==OrderTicket() || shortbuy02==OrderTicket() ||
          shortbuy03==OrderTicket() || shortbuy04==OrderTicket()) && 
          high_15_buffer[0]-OrderOpenPrice()>=0.0060 && high_15_buffer[0]-OrderOpenPrice()<=0.0099)
                       //stoploss=OrderOpenPrice()+7*Point可以賺25點
            OrderModify(OrderTicket(),Ask,OrderOpenPrice()+27*Point,OrderTakeProfit(),0,Blue);
      if (OrderType()==OP_BUY && (shortbuy01==OrderTicket() || shortbuy02==OrderTicket() ||
          shortbuy03==OrderTicket() || shortbuy04==OrderTicket()) && 
          high_15_buffer[0]-OrderOpenPrice()>=0.0100)
                       //stoploss=OrderOpenPrice()+7*Point可以賺55點
     ...
     if  (TimeCurrent()-OrderOpenTime()>=5000 &&  OrderType()==OP_SELL && (shortsell01==OrderTicket() || shortsell02==OrderTicket() ||
      ...
      if (OrderType()==OP_SELL && (shortsell01==OrderTicket() || shortsell02==OrderTicket() ||
          shortsell01==OrderTicket() || shortsell02==OrderTicket()) && 
            low_15_buffer[0]-OrderOpenPrice()<=-0.0060 && low_15_buffer[0]-OrderOpenPrice()>=-0.0099)
                                  //stoploss=OrderOpenPrice()-27*Point可以賺25點
            OrderModify(OrderTicket(),Bid,OrderOpenPrice()-27*Point,OrderTakeProfit(),0,Blue);
       if (OrderType()==OP_SELL && (shortsell01==OrderTicket() || shortsell02==OrderTicket() ||
          shortsell01==OrderTicket() || shortsell02==OrderTicket()) && 
            low_15_buffer[0]-OrderOpenPrice()<=-0.0100)
