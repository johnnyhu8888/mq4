...
// Alert (price,"   b= ",price1);
 double val;
 double val1;
 val=High[iHighest(NULL,0,MODE_HIGH,16,1)];

 val1=Low[iLowest(NULL,0,MODE_LOW,16,1)];
// Alert (val,"   low..",val1);  
...
//*** Buy  1  ***************************************************************************************** 
           ...
           (close_15_buffer[2]-open_15_buffer[2]>=0  &&  close_15_buffer[1]-open_15_buffer[1]>=0))
        {
              ...
              if ((ma05_15_buffer[1]-ma12_15_buffer[1]<=0 && ma05_15_buffer[1]-ma12_15_buffer[1]>=-0.0013 &&
                  ...
                  (close_15_buffer[2]-open_15_buffer[2]>=0  &&  close_15_buffer[1]-open_15_buffer[1]>=0))
                   ...
                   if   (TimeCurrent( )-OrderCloseTime( )<=1000 &&  magic==8801)
                         return;
                         else
 ...
 //*** Buy  2  ***************************************************************************************** 
                                   //40 bar ず痙单10翴紇絬 秖玡bar        
       if ((close_15_buffer[1]-open_15_buffer[1]>=0  &&  open_15_buffer[1]-low_15_buffer[1]>=0.0010 &&
           low_15_buffer[1]==M15lowest40bar  &&  volume_15_buffer[1]-(volume_15_buffer[2]*1.2)>=0))
        {
                                 //禦虫 bar [0]禴,は滦 close
            if   ((TimeCurrent( )-OrderCloseTime( )<=1000 &&  magic==8801) ||
                                         // //粇禴琿ぇ 紇絬 m15 k1k2k3<=-1 m30 k1<=-1
                 (k1_15_buffer[1]-d1_15_buffer[1]<=-1 && k2_15_buffer[1]-d2_15_buffer[1]<=-1 &&
                  k3_15_buffer[1]-d3_15_buffer[1]<=-1 && k1_30_buffer[1]-d1_30_buffer[1]<=-1))
                  return;
                  else
 ...
 //*** sell  2  ***************************************************************************************** 
                                   //40 bar ず痙单10翴紇絬   秖玡bar      
       if ((close_15_buffer[1]-open_15_buffer[1]<=0  &&  high_15_buffer[1]-open_15_buffer[1]>=0.0010 &&
           high_15_buffer[1]==M15highest40bar  &&  volume_15_buffer[1]-(volume_15_buffer[2]*1.2)>=0))
        {
              
                                           //芥虫 bar [0] 害,は滦 close
             if    ((TimeCurrent( )-OrderCloseTime( )<=1000 &&  magic==8801) ||
                                         //粇ど琿ぇ 紇絬 m15 k1k2k3>=1 m30 k1>=1
                   (k1_15_buffer[1]-d1_15_buffer[1]>=1 && k2_15_buffer[1]-d2_15_buffer[1]>=1 &&
                    k3_15_buffer[1]-d3_15_buffer[1]>=1 && k1_30_buffer[1]-d1_30_buffer[1]>=1))
...
//***  CLOSE Sell   1 ********************************************************************************** 
 ...
 //***  CLOSE Sell   2 ********************************************************************************** 
                                //40 bar ず痙单10翴紇絬 秖玡bar        
          if ((close_05_buffer[1]-open_15_buffer[1]>=0  &&  open_05_buffer[1]-low_15_buffer[1]>=0.0010 &&
              low_15_buffer[1]==M15lowest40bar  &&  volume_15_buffer[1]-(volume_15_buffer[2]*1.2)>=0))
