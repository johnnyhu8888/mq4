...
//*** Sell  1  ********************************************************************************************************************** 
    ...
    if (OrdersTotal()<=1)   
     ...
     if   ((k1_05_buffer[1]-k1_05_buffer[2]<=-3 &&  k2_05_buffer[1]-k2_05_buffer[2]<=-1 && k3_05_buffer[1]-k3_05_buffer[2]<=0  &&
              ...
              (k1_240_buffer[0]-k1_240_buffer[1]>=3 &&  k2_240_buffer[0]-k2_240_buffer[1]>=1 && k3_240_buffer[0]-k3_240_buffer[1]>=0) ||
               ...
               (OrdersTotal()==1 && (shortbuy01==OrderTicket() || shortbuy02==OrderTicket() ||shortbuy03==OrderTicket() ||
                ...
                shortsell01=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-250*Point,"My order #S1",4401,0,Green);
                                                                        ...
                                                                        //秨虫30だぃ笆
      if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.01300 &&  k1_240_buffer[1]-k1_240_buffer[2]>=3 &&  k2_240_buffer[1]-k2_240_buffer[2]>=1 && k3_240_buffer[1]-k3_240_buffer[2]>=0)
            OrderModify(OrderTicket(),Ask,OrderOpenPrice()+700*Point,OrderOpenPrice()+1800*Point,0,Blue);
         else
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00800 &&  k1_60_buffer[1]-k1_60_buffer[2]>=3 &&  k2_60_buffer[1]-k2_60_buffer[2]>=1 && k3_60_buffer[1]-k3_60_buffer[2]>=0)
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+500*Point,OrderOpenPrice()+1000*Point,0,Blue);
         else
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00500 &&  k1_30_buffer[1]-k1_30_buffer[2]>=3 &&  k2_30_buffer[1]-k2_30_buffer[2]>=1 && k3_30_buffer[1]-k3_30_buffer[2]>=0)
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+300*Point,OrderOpenPrice()+750*Point,0,Blue);
         else
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00400 && k1_15_buffer[1]-k1_15_buffer[2]>=3 &&  k2_15_buffer[1]-k2_15_buffer[2]>=1 && k3_15_buffer[1]-k3_15_buffer[2]>=0)
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+200*Point,OrderOpenPrice()+550*Point,0,Blue);
         else
            if (OrderType()==OP_BUY && shortbuy01==OrderTicket() && high_05_buffer[0]-OrderOpenPrice()>=0.00200) // && TimeCurrent()-OrderOpenTime()>=2000)
                             //stoploss=OrderOpenPrice()+40*Point寥4翴
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+50*Point,OrderTakeProfit(),0,Blue);
         else
            if (OrderType()==OP_BUY && shortbuy01==OrderTicket() && high_05_buffer[0]-OrderOpenPrice()>=0.00100) // && TimeCurrent()-OrderOpenTime()>=2000)
      ...
      if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.01300 &&  k1_240_buffer[1]-k1_240_buffer[2]<=-3 &&  k2_240_buffer[1]-k2_240_buffer[2]<=-1 && k3_240_buffer[1]-k3_240_buffer[2]<=0)
            OrderModify(OrderTicket(),Bid,OrderOpenPrice()-700*Point,OrderOpenPrice()-1800*Point,0,Blue);
         else       
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00800 &&  k1_60_buffer[1]-k1_60_buffer[2]<=-3 &&  k2_60_buffer[1]-k2_60_buffer[2]<=-1 && k3_60_buffer[1]-k3_60_buffer[2]<=0)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-500*Point,OrderOpenPrice()-1000*Point,0,Blue);
         else
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00500 &&  k1_30_buffer[1]-k1_30_buffer[2]<=-3 &&  k2_30_buffer[1]-k2_30_buffer[2]<=-1 && k3_30_buffer[1]-k3_30_buffer[2]<=0)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-300*Point,OrderOpenPrice()-750*Point,0,Blue);
         else
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00400 &&  k1_15_buffer[1]-k1_15_buffer[2]<=-3 &&  k2_15_buffer[1]-k2_15_buffer[2]<=-1 && k3_15_buffer[1]-k3_15_buffer[2]<=0)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-200*Point,OrderOpenPrice()-550*Point,0,Blue);
         else
            if (OrderType()==OP_SELL && shortsell01==OrderTicket() && low_05_buffer[0]-OrderOpenPrice()<=-0.00200) // && TimeCurrent()-OrderOpenTime()>=2000)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-50*Point,OrderTakeProfit(),0,Blue);
         else
            if (OrderType()==OP_SELL &&  shortsell01==OrderTicket() && low_05_buffer[0]-OrderOpenPrice()<=-0.00100) // && TimeCurrent()-OrderOpenTime()>=2000)
