...
//     {              
     ...
     if  (((k1_15_buffer[1]-k1_15_buffer[2]>=3 &&  k2_15_buffer[1]-k2_15_buffer[2]>=1 && k3_15_buffer[1]-k3_15_buffer[2]>=0 &&
           ...
           (k1_30_buffer[1]-k1_30_buffer[2]>=3 &&  k2_30_buffer[1]-k2_30_buffer[2]>=1 && k3_30_buffer[1]-k3_30_buffer[2]>=0 &&
            k1_30_buffer[1]<=50 &&   k2_30_buffer[1]<=50 && k3_30_buffer[1]<=50))) 
            if    (k1_05_buffer[1]-d1_05_buffer[2]>=3 &&   k1_05_buffer[1]<=50)                               
    
            if   ((k1_60_buffer[0]-k1_60_buffer[1]<=-3 &&  k2_60_buffer[0]-k2_60_buffer[1]<=-3 && k3_60_buffer[0]-k3_60_buffer[1]<=-3) ||
                  (k1_240_buffer[0]-k1_240_buffer[1]<=-3 &&  k2_240_buffer[0]-k2_240_buffer[1]<=-3 && k3_240_buffer[0]-k3_240_buffer[1]<=-3) ||
 //┮Τ–眎虫冈灿orderticket北恨ぃ璶Τ虫猵祇ネ                   
                  ...
                  return;
                   ...
                   longbuy01=OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+800*Point,"My order #3",8803,0,Yellow );
     }           
//*** long Buy  2  ***************************************************************************************** 
   ...
   if (OrdersTotal()<=1)   
     ...
     if  (((k1_60_buffer[1]-k1_60_buffer[2]>=3 &&  k2_60_buffer[1]-k2_60_buffer[2]>=1 && k3_60_buffer[1]-k3_60_buffer[2]>=0 &&
            k1_60_buffer[1]<=50 &&   k2_60_buffer[1]<=50 && k3_60_buffer[1]<=50) ||
           (k1_240_buffer[1]-k1_240_buffer[2]>=3 &&  k2_240_buffer[1]-k2_240_buffer[2]>=1 && k3_240_buffer[1]-k3_240_buffer[2]>=0 &&
            k1_240_buffer[1]<=60 &&   k2_240_buffer[1]<=50 && k3_240_buffer[1]<=50))) 
            if    (k1_15_buffer[1]-d1_15_buffer[2]>=3 &&   k1_15_buffer[1]<=50)                               
 ...
 //┮Τ–眎虫冈灿orderticket北恨ぃ璶Τ虫猵祇ネ                   
            ...
            longbuy02=OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+1800*Point,"My order #4",8804,0,Yellow );
     }  
    ...
    if (OrdersTotal()<=1)   
     {              
      ...
      if (((k1_15_buffer[1]-k1_15_buffer[2]<=-3 &&  k2_15_buffer[1]-k2_15_buffer[2]<=-1 && k3_15_buffer[1]-k3_15_buffer[2]<=0 &&
          ...
          if ((k1_60_buffer[1]-k1_60_buffer[2]>=3 &&  k2_60_buffer[1]-k2_60_buffer[2]>=3 && k3_60_buffer[1]-k3_60_buffer[2]>=3) ||
              (k1_240_buffer[1]-k1_240_buffer[2]>=3 &&  k2_240_buffer[1]-k2_240_buffer[2]>=3 && k3_240_buffer[1]-k3_240_buffer[2]>=3) ||
...
//┮Τ–眎虫冈灿orderticket北恨ぃ璶Τ虫猵祇ネ                   
              ...
              longsell01=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-800*Point,"My order #S3",4403,0,Green);
     }
 ...
 //┮Τ–眎虫冈灿orderticket北恨ぃ璶Τ虫猵祇ネ                   
              if    ((k1_240_buffer[1]-d1_240_buffer[1]>=7 && k2_240_buffer[1]-d2_240_buffer[1]>=7 && k3_240_buffer[1]-d3_240_buffer[1]>=7 &&
                  ...
                  longsell02=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-1800*Point,"My order #S4",4404,0,Green);
     }      
          ...
          if ((k1_15_buffer[0]-k1_15_buffer[1]<=-3 &&  k2_15_buffer[0]-k2_15_buffer[1]<=-1 && k3_15_buffer[0]-k3_15_buffer[1]<=0) ||
              ...
              (k1_240_buffer[0]-k1_240_buffer[1]<=-3 &&  k2_240_buffer[0]-k2_240_buffer[1]<=-1 && k3_240_buffer[0]-k3_240_buffer[1]<=0) ||
                ...
                (OrdersTotal()==1 && (shortbuy01==OrderTicket() || shortbuy02==OrderTicket() ||shortbuy03==OrderTicket() ||
                 ...
                 shortbuy01=OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+250*Point,"My order #1",8801,0,Yellow );
     }  
   ...
   for (cnt=0;cnt<OrdersTotal();cnt++)
     {
       OrderSelect(cnt,SELECT_BY_POS,MODE_TRADES);
                              ...
                              //秨虫10だず硉,搭ぶ穕ア
         if   (OrderType()==OP_BUY && OrderOpenTime()-TimeCurrent()<=600 && low_05_buffer[0]-OrderOpenPrice()<=-0.00120 && low_05_buffer[0]-OrderOpenPrice()>=-0.00130)
            ...
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.01300 && high_05_buffer[0]-OrderOpenPrice()<=0.01320 &&  k1_240_buffer[1]-k1_240_buffer[2]>=3 &&  k2_240_buffer[1]-k2_240_buffer[2]>=1 && k3_240_buffer[1]-k3_240_buffer[2]>=0)
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+700*Point,OrderOpenPrice()+1800*Point,0,Blue);
         else
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00800 && high_05_buffer[0]-OrderOpenPrice()<=0.00820 && k1_60_buffer[1]-k1_60_buffer[2]>=3 &&  k2_60_buffer[1]-k2_60_buffer[2]>=1 && k3_60_buffer[1]-k3_60_buffer[2]>=0)
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+500*Point,OrderOpenPrice()+1000*Point,0,Blue);
         else
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00500 && high_05_buffer[0]-OrderOpenPrice()<=0.00520 && k1_30_buffer[1]-k1_30_buffer[2]>=3 &&  k2_30_buffer[1]-k2_30_buffer[2]>=1 && k3_30_buffer[1]-k3_30_buffer[2]>=0)
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+300*Point,OrderOpenPrice()+750*Point,0,Blue);
         else
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00400 && high_05_buffer[0]-OrderOpenPrice()<=0.00420 && k1_15_buffer[1]-k1_15_buffer[2]>=3 &&  k2_15_buffer[1]-k2_15_buffer[2]>=1 && k3_15_buffer[1]-k3_15_buffer[2]>=0)
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+200*Point,OrderOpenPrice()+550*Point,0,Blue);
         else
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00220 && high_05_buffer[0]-OrderOpenPrice()<=0.00230) // && TimeCurrent()-OrderOpenTime()>=2000)
                             //stoploss=OrderOpenPrice()+40*Point寥4翴
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+100*Point,OrderTakeProfit(),0,Blue);
         else
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00110 && high_05_buffer[0]-OrderOpenPrice()<=0.00120) // && TimeCurrent()-OrderOpenTime()>=2000)
                             //stoploss=OrderOpenPrice()-40*Point莲3翴
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+50*Point,OrderTakeProfit(),0,Blue);
              
     
                              //秨虫10だず硉ど,搭ぶ穕ア
         if   (OrderType()==OP_SELL && OrderOpenTime()-TimeCurrent()<=600 && high_05_buffer[0]-OrderOpenPrice()>=0.00120 && high_05_buffer[0]-OrderOpenPrice()<=0.00130)
            ...
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.01300 && low_05_buffer[0]-OrderOpenPrice()>=-0.01320 && k1_240_buffer[1]-k1_240_buffer[2]<=-3 &&  k2_240_buffer[1]-k2_240_buffer[2]<=-1 && k3_240_buffer[1]-k3_240_buffer[2]<=0)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-700*Point,OrderOpenPrice()-1800*Point,0,Blue);
         else       
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00800 && low_05_buffer[0]-OrderOpenPrice()>=-0.00820 && k1_60_buffer[1]-k1_60_buffer[2]<=-3 &&  k2_60_buffer[1]-k2_60_buffer[2]<=-1 && k3_60_buffer[1]-k3_60_buffer[2]<=0)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-500*Point,OrderOpenPrice()-1000*Point,0,Blue);
         else
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00500 && low_05_buffer[0]-OrderOpenPrice()>=-0.00520 && k1_30_buffer[1]-k1_30_buffer[2]<=-3 &&  k2_30_buffer[1]-k2_30_buffer[2]<=-1 && k3_30_buffer[1]-k3_30_buffer[2]<=0)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-300*Point,OrderOpenPrice()-750*Point,0,Blue);
         else
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00400 && low_05_buffer[0]-OrderOpenPrice()>=-0.00420 && k1_15_buffer[1]-k1_15_buffer[2]<=-3 &&  k2_15_buffer[1]-k2_15_buffer[2]<=-1 && k3_15_buffer[1]-k3_15_buffer[2]<=0)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-200*Point,OrderOpenPrice()-550*Point,0,Blue);
         else
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00220 && low_05_buffer[0]-OrderOpenPrice()>=-0.00230) // && TimeCurrent()-OrderOpenTime()>=2000)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-100*Point,OrderTakeProfit(),0,Blue);
         else
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00110 && low_05_buffer[0]-OrderOpenPrice()>=-0.00120) // && TimeCurrent()-OrderOpenTime()>=2000)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-50*Point,OrderTakeProfit(),0,Blue);
       
    
//    if (OrderType()==OP_BUY && (longbuy01==OrderTicket() || longbuy02==OrderTicket()) && high_05_buffer[0]-OrderOpenPrice()>=0.00300 && high_05_buffer[0]-OrderOpenPrice()<=0.00310) // && TimeCurrent()-OrderOpenTime()>=2000)
                             //stoploss=OrderOpenPrice()+40*Point寥4翴
//                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+100*Point,OrderTakeProfit(),0,Blue);
//         else
//            if (OrderType()==OP_BUY && (longbuy01==OrderTicket() || longbuy02==OrderTicket()) && high_05_buffer[0]-OrderOpenPrice()>=0.00150 && high_05_buffer[0]-OrderOpenPrice()<=0.00160) // && TimeCurrent()-OrderOpenTime()>=2000)
                             //stoploss=OrderOpenPrice()-40*Point莲3翴
//                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()-50*Point,OrderTakeProfit(),0,Blue);
     
//            if (OrderType()==OP_SELL && (longsell01==OrderTicket() || longsell02==OrderTicket()) && low_05_buffer[0]-OrderOpenPrice()<=-0.00300 && low_05_buffer[0]-OrderOpenPrice()>=-0.00310) // && TimeCurrent()-OrderOpenTime()>=2000)
//                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-100*Point,OrderTakeProfit(),0,Blue);
//         else
//            if (OrderType()==OP_SELL && (longsell01==OrderTicket() || longsell02==OrderTicket()) && low_05_buffer[0]-OrderOpenPrice()<=-0.00150 && low_05_buffer[0]-OrderOpenPrice()>=-0.00160) // && TimeCurrent()-OrderOpenTime()>=2000)
//                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()+50*Point,OrderTakeProfit(),0,Blue);
