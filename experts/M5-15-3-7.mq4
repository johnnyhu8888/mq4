   ...
   for (cnt=0;cnt<OrdersTotal();cnt++)
     {
       OrderSelect(cnt,SELECT_BY_POS,MODE_TRADES);
                              ...
                              //秨虫10だず硉,搭ぶ穕ア
         if   (OrderType()==OP_BUY && TimeCurrent()-OrderOpenTime()<=600 && OrderOpenPrice()-high_05_buffer[0]<=0.00060 && OrderOpenPrice()-high_05_buffer[1]<=0.00060 &&
    ...
    Comment("aaaaaaaaaaa=",TimeCurrent()-OrderOpenTime());  
    //     else                                                                     
            ...
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00100 && high_05_buffer[0]-OrderOpenPrice()<=0.00110) // && TimeCurrent()-OrderOpenTime()>=2000)
         ...
         if   (OrderType()==OP_SELL && OrderOpenTime()-TimeCurrent()<=600 && high_05_buffer[0]-OrderOpenPrice()>=0.00120 && high_05_buffer[0]-OrderOpenPrice()<=0.00130)
            ...
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00100 && low_05_buffer[0]-OrderOpenPrice()>=-0.00110) // && TimeCurrent()-OrderOpenTime()>=2000)
