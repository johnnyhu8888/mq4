...
//           if    (OrderType()==OP_BUY && (OrderMagicNumber( )==8801 || OrderMagicNumber( )==88011) &&
           if    (OrderType()==OP_BUY && //OrderMagicNumber( )!=8806 &&
                  high_15_buffer[0]-OrderOpenPrice()>=0.01000 && high_05_buffer[0]-OrderOpenPrice()<=0.01010) 
                             //stoploss=OrderOpenPrice()+40*Point可以賺4點
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+380*Point,OrderTakeProfit(),0,Blue);
//           if    (OrderType()==OP_BUY && (OrderMagicNumber( )==8801 || OrderMagicNumber( )==88011) &&
     //      if    (OrderType()==OP_BUY && //OrderMagicNumber( )!=8806 &&
...
//           if    (OrderType()==OP_BUY && (OrderMagicNumber( )==8807 || OrderMagicNumber( )==8806) &&
           if    (OrderType()==OP_BUY &&// OrderMagicNumber( )==8806 &&
                  high_15_buffer[0]-OrderOpenPrice()>=0.00500 && high_05_buffer[0]-OrderOpenPrice()<=0.00510) 
                             //stoploss=OrderOpenPrice()+40*Point可以賺4點
          ...
          if    (OrderType()==OP_BUY && (OrderMagicNumber( )!=8807 || OrderMagicNumber( )!=8809) &&  //       if    (OrderType()==OP_BUY && // OrderMagicNumber( )==8806 &&
                 high_15_buffer[0]-OrderOpenPrice()>=0.00290 && high_05_buffer[0]-OrderOpenPrice()<=0.00300) 
...
//           if    (OrderType()==OP_SELL && (OrderMagicNumber( )==4401 || OrderMagicNumber( )==44011) &&
           if    (OrderType()==OP_SELL &&// OrderMagicNumber( )!=4406 &&
                  low_15_buffer[0]-OrderOpenPrice()<=-0.01000 && low_05_buffer[0]-OrderOpenPrice()>=-0.01010) 
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-380*Point,OrderTakeProfit(),0,Blue);
//           if    (OrderType()==OP_SELL && (OrderMagicNumber( )==4401 || OrderMagicNumber( )==44011) &&
       //    if    (OrderType()==OP_SELL && //OrderMagicNumber( )!=4406 &&
...
//           if    (OrderType()==OP_SELL && (OrderMagicNumber( )==4401 || OrderMagicNumber( )==44011) &&
 ...
 //          if    (OrderType()==OP_SELL && (OrderMagicNumber( )==4407 || OrderMagicNumber( )==4406) &&
           if    (OrderType()==OP_SELL && //OrderMagicNumber( )==4406 &&
                  low_15_buffer[0]-OrderOpenPrice()<=-0.00500 && low_05_buffer[0]-OrderOpenPrice()>=-0.00510) 
     ...
     //     if    (OrderType()==OP_SELL && // OrderMagicNumber( )==4406 &&
                  low_15_buffer[0]-OrderOpenPrice()<=-0.00290 && low_05_buffer[0]-OrderOpenPrice()>=-0.00300) 
...
//    if  ( OrdersTotal()>0 && OrderMagicNumber()==4401) 
 ...
 // H4  ma05,ma08,ma12,ma45,ma90,ma180,ma360,ma650 全斜上    
                   (ma05_240_buffer[1]-ma05_240_buffer[2]>=0.00010 &&  ma08_240_buffer[1]-ma08_240_buffer[2]>=0.00010 && 
                    ma12_240_buffer[1]-ma12_240_buffer[2]>=0.00010 &&  ma45_240_buffer[1]-ma45_240_buffer[2]>=0.00010 && 
...
//用在轉折所以排除條件較少      
 ...
 //H1 K1,K2或K3向上
           (k1_60_buffer[1]-k1_60_buffer[2]>=1 &&  (k2_60_buffer[1]-k2_60_buffer[2]>=1  ||  k3_60_buffer[1]-k3_60_buffer[2]>=1  && k2_60_buffer[1]<=30)))
            OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-30*Point,Ask+6900*Point,"My order #B6",8806,0,Aqua);
...
//m240之ma05與ma12差距在40點內
 ...
 // H4 [0]最低已達[1]或[2]    或[2]或[3]  下影線之半      
          ((low_240_buffer[0]-((low_240_buffer[1]-close_240_buffer[1])/2<=-0.00010 || (low_240_buffer[2]-close_240_buffer[2])/2<=-0.00010)) ||
           (low_240_buffer[0]-((low_240_buffer[2]-close_240_buffer[2])/2<=-0.00010 || (low_240_buffer[3]-close_240_buffer[3])/2<=-0.00010)))))
            OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-300*Point,Ask+6900*Point,"My order #B12",8812,0,Yellow);
 ...
 // H4 k2,k3皆上升,,且在30下 
           (k2_240_buffer[1]-k2_240_buffer[2]>=1 && k2_240_buffer[1]<=30 &&
            k3_240_buffer[1]-k3_240_buffer[2]>=1 && k3_240_buffer[1]<=30)) 
            OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-300*Point,Ask+6900*Point,"My order #B13",8813,0,Yellow);
...
//用在轉折所以排除條件較少      
 ...
 //H1 K1,K2或K3向下
           (k1_60_buffer[1]-k1_60_buffer[2]<=-1 &&  (k2_60_buffer[1]-k2_60_buffer[2]<=-1  ||  k3_60_buffer[1]-k3_60_buffer[2]<=-1  && k2_60_buffer[1]>=70)))
            OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+300*Point,Bid-6900*Point,"My order #S6",4406,0,Green);
...
//用在轉折所以排除條件較少 做短單     
 ...
 // H4 [0]最高已達[1]或[2]  [2]或[3] 上影線之半         
          ((high_240_buffer[0]-((high_240_buffer[1]-close_240_buffer[1])/2>=0.00010 || (high_240_buffer[2]-close_240_buffer[2])/2>=0.00010)) ||
           (high_240_buffer[0]-((high_240_buffer[2]-close_240_buffer[2])/2>=0.00010 || (high_240_buffer[3]-close_240_buffer[3])/2>=0.00010)))))
            OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+300*Point,Bid-6900*Point,"My order #S12",4412,0,Green);
 ...
 // H4 k2,k3皆下降,,且在70上
           (k2_240_buffer[1]-k2_240_buffer[2]<=-1 && k2_240_buffer[1]>=70 &&
            k3_240_buffer[1]-k3_240_buffer[2]<=-1 && k3_240_buffer[1]>=70)) 
            OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+300*Point,Bid-6900*Point,"My order #S13",4413,0,Green);
