   ...
   int _GetLastError = 0, _OrdersTotal = OrdersTotal();
    ...
    for ( int z = _OrdersTotal - 1; z >= 0; z -- )
    {
        ...
        if ( !OrderSelect( z, SELECT_BY_POS ) )
        {
            ...
            Print( "OrderSelect( ", z, ", SELECT_BY_POS ) - 錯誤#", _GetLastError );
            continue;
        }
        ...
        //---- 如果MagicNumber 不等于 _MagicNumber, 忽略此倉位
        ...
        if ( OrderType() == OP_BUY )
        {
                  ...
                  NormalizeDouble( MACD_2, Digits + 1 ) >= 0.0    )
            {
                ...
                {
                    _GetLastError = GetLastError();
                    Alert( "錯誤 OrderClose # ", _GetLastError );
                    return(-1);
                }
            }
            //---- 如果警報沒有改變，停止: 開新倉位尚早
  ...
  if ( OrdersTotal()>0 && OrderMagicNumber() == 8805 ) 
      return;
                  ...
                  if   ((d2_30_buffer[1]<=55 && d3_30_buffer[1]<=55 &&
                    ...
                    if    (k1_05_buffer[1]-k1_05_buffer[2]>=2 && k1_05_buffer[1]<=20 &&
                          (k2_05_buffer[1]-k2_05_buffer[2]>=0 || k3_05_buffer[1]-k3_05_buffer[2]>=0)) 
                           shortbuy01=OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+450*Point,"My order #1",8805,0,Yellow );
 ...
 //*** long Sell  2  ********************************************************************************************************************** 
  ...
  if ( OrdersTotal()>0 && OrderMagicNumber() == 4404 ) 
     ...
     if   ((k1_60_buffer[1]>=85 &&   k2_60_buffer[1]>=85 && k3_60_buffer[1]>=85 &&  
          ...
          ((macdmainline_240_buffer[2]-macdsignalline_240_buffer[2])-(macdmainline_240_buffer[3]-macdsignalline_240_buffer[3]))<=-0.000500 &&  
            close_240_buffer[1]-open_240_buffer[1]>=0.00001))
            longsell04=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+300*Point,Bid-9000*Point,"My order #LS4",4404,0,Blue);
            ...
            if   ((d2_60_buffer[1]>=45 && d3_60_buffer[1]>=45 &&
                 ...
                 ((k1_240_buffer[1]-d1_240_buffer[1])+(k2_240_buffer[1]-d2_240_buffer[1])+(k3_240_buffer[1]-d3_240_buffer[1]))<50) ||
                   ...
                   (macdmainline_240_buffer[1]-macdsignalline_240_buffer[1]<=-0.002000) ||
                    ...
                    if    (k1_05_buffer[1]-k1_05_buffer[2]<=-2 && k1_05_buffer[1]>=80 &&
                          (k2_05_buffer[1]-k2_05_buffer[2]<=0 || k3_05_buffer[1]-k3_05_buffer[2]<=0)) 
                           shortsell01=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-450*Point,"My order #S1",4405,0,Green);
