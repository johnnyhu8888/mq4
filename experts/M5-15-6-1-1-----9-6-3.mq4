...
//本段已有作用 
            if    (OrderType()==OP_BUY && OrderMagicNumber( )==8806)  // || OrderMagicNumber( )==4402) &&
              if  (TimeCurrent()- OrderOpenTime()>=700 && TimeCurrent()- OrderOpenTime()<=2000 &&
                  (close_05_buffer[1]-OrderOpenPrice()<=0.00010 && close_05_buffer[2]-OrderOpenPrice()<=0.00050) || 
                           //[0]的判斷避免反轉
                  (close_05_buffer[0]-OrderOpenPrice()<=0.00050) || 
                            //超過12分後 [0]急速反轉 close[0]<close[1] && close[2]
                 (TimeCurrent()- OrderOpenTime()>=701 &&
                  close_05_buffer[1]-OrderOpenPrice()<=0.00050 && close_05_buffer[1]-close_05_buffer[2]<=-0.00010 &&
                  close_05_buffer[0]-close_05_buffer[3]<=-0.00010))
                            //開單後13分內 [0]直接反轉 收黑大於13點
 //                (TimeCurrent()- OrderOpenTime()>=1 &&  TimeCurrent()- OrderOpenTime()<=700 && 
 //                 low_05_buffer[0]-OrderOpenPrice()<=-0.00010))
    
                   OrderClose(OrderTicket(),lots,Bid,1,Gold);    
 
            if    (OrderType()==OP_BUY && (OrderMagicNumber( )==8801 || OrderMagicNumber( )==88011))
                                //單位為秒,33分~~83分
              if ((TimeCurrent()- OrderOpenTime()>=2000 && TimeCurrent()- OrderOpenTime()<=5000 &&
                  (close_15_buffer[1]-OrderOpenPrice()<=0.00010 && close_15_buffer[2]-OrderOpenPrice()<=0.00050) ||
                           //時間內[0]的判斷避免反轉
                  (close_15_buffer[0]-OrderOpenPrice()<=0.00050)) ||
                            //超過83分後 [0]急速反轉 close[0]<close[1] && close[2]
                 (TimeCurrent()- OrderOpenTime()>=5001 &&
                  close_15_buffer[1]-OrderOpenPrice()<=0.00050 && close_15_buffer[1]-close_15_buffer[2]<=-0.00010 &&
                  close_15_buffer[1]-close_15_buffer[3]<=-0.00010))
                  OrderClose(OrderTicket(),lots,Bid,1,Gold); 
                            //開單後33分內 [0]直接反轉 收黑大於13點
            if   (TimeCurrent()- OrderOpenTime()>=1 &&  TimeCurrent()- OrderOpenTime()<=1999 && 
                ((close_15_buffer[0]-OrderOpenPrice()<=-0.00160)||(close_15_buffer[1]-OrderOpenPrice()<=-0.00160)))
                  OrderClose(OrderTicket(),lots,Bid,1,Gold); 
            ...
            if    (OrderType()==OP_SELL &&  OrderMagicNumber( )==4406)  // || OrderMagicNumber( )==4402) &&
              if  (TimeCurrent()- OrderOpenTime()>=700 && TimeCurrent()- OrderOpenTime()<=2000 &&
                  ...
                  (close_05_buffer[0]-OrderOpenPrice()>=-0.00050) || 
                     //超過12分後 [0]急速反轉 close[0]<close[1] && close[2]
                 (TimeCurrent()- OrderOpenTime()>=701 &&
                  close_05_buffer[1]-OrderOpenPrice()>=-0.00010 && close_05_buffer[1]-close_05_buffer[2]>=0.00010 &&
                  close_05_buffer[1]-close_05_buffer[3]>=0.00010))
                            //開單後13分內 [0]直接反轉 收黑大於13點
 //                (TimeCurrent()- OrderOpenTime()>=1 &&  TimeCurrent()- OrderOpenTime()<=700 && 
//                  high_05_buffer[0]-OrderOpenPrice()>=0.00010))
            ...
            if    (OrderType()==OP_SELL && (OrderMagicNumber( )==4401 || OrderMagicNumber( )==44011))
              ...
              if  (TimeCurrent()- OrderOpenTime()>=2000 && TimeCurrent()- OrderOpenTime()<=5000 &&
                  ...
                  (close_15_buffer[0]-OrderOpenPrice()>=-0.00050) || 
                             //超過83分後 [0]急速反轉 close[0]<close[1] && close[2]
                 (TimeCurrent()- OrderOpenTime()>=5001 &&
                  close_15_buffer[1]-OrderOpenPrice()>=-0.00010 && close_15_buffer[1]-close_15_buffer[2]>=0.00010 &&
                  close_15_buffer[1]-close_15_buffer[3]>=0.00010))
                 OrderClose(OrderTicket(),lots,Ask,1,Gold);    
                            //開單後33分內 [0]直接反轉 收黑大於13點
           if    (TimeCurrent()- OrderOpenTime()>=1 &&  TimeCurrent()- OrderOpenTime()<=1999 && 
               ...
               if   ((d2_30_buffer[1]<=55 && d3_30_buffer[1]<=55 &&
                    ...
                    ((k1_240_buffer[1]-d1_240_buffer[1])+(k2_240_buffer[1]-d2_240_buffer[1])+(k3_240_buffer[1]-d3_240_buffer[1]))>50) ||
                      ...
                      (macdmainline_240_buffer[1]-macdsignalline_240_buffer[1]>=0.001000) ||
                       ...
                       //ma05 持平狀態 
                  (ma05_15_buffer[1]-ma05_15_buffer[6]<=0.00020 && ma05_15_buffer[1]-ma05_15_buffer[6]>=-0.00020 && 
                             //k1,k2,k3  任一持平
                 ((k1_15_buffer[1]-k1_15_buffer[6]<=2 && k1_15_buffer[1]-k1_15_buffer[6]>=-2 ||
                  ...
                  (close_15_buffer[1]-close_15_buffer[20]>=0.00200 ||  close_15_buffer[1]-close_15_buffer[40]>=0.00200)) || 
                       ...
                       //ma05 持平狀態 
                  (ma05_30_buffer[1]-ma05_30_buffer[6]<=0.00020 && ma05_30_buffer[1]-ma05_30_buffer[6]>=-0.00020 && 
                             //k1,k2,k3  任一持平
                 ((k1_30_buffer[1]-k1_30_buffer[6]<=2 && k1_30_buffer[1]-k1_30_buffer[6]>=-2 ||
                  ...
                  (close_30_buffer[1]-close_30_buffer[20]>=0.00200 ||  close_30_buffer[1]-close_30_buffer[40]>=0.00200)) || 
                       ...
                       //ma05 持平狀態 
                  (ma05_60_buffer[1]-ma05_60_buffer[6]<=0.00020 && ma05_60_buffer[1]-ma05_60_buffer[6]>=-0.00020 && 
                             //k1,k2,k3  任一持平
...
//*** close Buy  2  ***************************************************************************************** 
 //   if ( OrdersTotal()>0 && OrderMagicNumber() == 4406 ) 
    ...
    if    ( OrdersTotal()>0 && (OrderMagicNumber()!=4401 ||OrderMagicNumber()!=44011)) 

      if   (k1_05_buffer[1]-k1_05_buffer[2]<=-3 &&  k1_05_buffer[1]>=85 &&
            k2_05_buffer[1]>=80 && k3_05_buffer[1]>=80 &&
            k1_240_buffer[1]-k1_240_buffer[2]<=-1 &&      
...
if ( OrdersTotal()>0 && (OrderMagicNumber() != 8801 || OrderMagicNumber() != 88011))    
