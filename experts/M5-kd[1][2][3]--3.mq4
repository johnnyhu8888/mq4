        ...
        if  (k1_05_buffer[1]-k1_05_buffer[2]>=1 && k2_05_buffer[1]-k2_05_buffer[2]>=1 && k3_05_buffer[1]-k3_05_buffer[2]>=0)
         {
             if  (close_05_buffer[1]-open_05_buffer[1]>=0.00040 &&  close_05_buffer[2]-open_05_buffer[2]<=-0.00040 &&
                 ...
                 shortsell01=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-400*Point,"My order #S1",4401,0,Green);

   //               shortbuy01=OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+400*Point,"My order #1",8801,0,Yellow );
         } 
     }           
   ...
   for (cnt=0;cnt<OrdersTotal();cnt++)
     {
       ...
       if (OrderType()==OP_BUY && (shortbuy01==OrderTicket() || shortbuy02==OrderTicket() || 
           ...
           high_05_buffer[0]-OrderOpenPrice()>=0.00100 && high_05_buffer[0]-OrderOpenPrice()<=0.00200 )
                       //stoploss=OrderOpenPrice()+7*Point寥5翴
            OrderModify(OrderTicket(),Ask,OrderOpenPrice()+70*Point,OrderTakeProfit(),0,Blue);
      if (OrderType()==OP_BUY && (shortbuy01==OrderTicket() || shortbuy02==OrderTicket() ||
          shortbuy03==OrderTicket() || shortbuy04==OrderTicket()) && 
          high_05_buffer[0]-OrderOpenPrice()>=0.00200 && high_05_buffer[0]-OrderOpenPrice()<=0.00990)
                       //stoploss=OrderOpenPrice()+7*Point寥25翴
            OrderModify(OrderTicket(),Ask,OrderOpenPrice()+270*Point,OrderTakeProfit(),0,Blue);
            
                                                 //Τ2Ωэ,ゲ斗р,璶絛瞅 
                                    //ノ low[1] 粇畉ぃ穦び   斗﹚Μ堵玥はΜ穦э    
   ...
   //         OrderModify(OrderTicket(),OrderOpenPrice(),M15lowest16bar+50*Point,OrderTakeProfit(),0,Blue);
      ...
      if (OrderType()==OP_SELL && (shortsell01==OrderTicket() || shortsell02==OrderTicket() ||
          shortsell03==OrderTicket() || shortsell04==OrderTicket()) && 
            low_05_buffer[0]-OrderOpenPrice()<=-0.00100 && low_05_buffer[0]-OrderOpenPrice()>=-0.00200)
                                  //stoploss=OrderOpenPrice()-7*Point寥5翴
            OrderModify(OrderTicket(),Bid,OrderOpenPrice()-70*Point,OrderTakeProfit(),0,Blue);
      if (OrderType()==OP_SELL && (shortsell01==OrderTicket() || shortsell02==OrderTicket() ||
          shortsell01==OrderTicket() || shortsell02==OrderTicket()) && 
            low_05_buffer[0]-OrderOpenPrice()<=-0.00200 && low_15_buffer[0]-OrderOpenPrice()>=-0.00990)
                                  //stoploss=OrderOpenPrice()-27*Point寥25翴
