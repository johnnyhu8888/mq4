         ...
         ((k1_05_buffer[1]>=50 &&   k2_05_buffer[1]>=50 && k3_05_buffer[1]>=50) ||
          (k2_05_buffer[1]-d2_05_buffer[1]<=-15 && k3_05_buffer[1]-d3_05_buffer[1]<=-15)))   
           ...
           shortsell01=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-450*Point,"My order #S1",4401,0,Green);
                                                                        ...
                                                                        //秨虫50だぃ笆
      if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00010 && TimeCurrent()-OrderOpenTime()>=3000)
                       //stoploss=OrderOpenPrice()+40*Point寥4翴
            OrderModify(OrderTicket(),Ask,OrderOpenPrice()+40*Point,OrderTakeProfit(),0,Blue);
                  
    if  (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00010 && TimeCurrent()-OrderOpenTime()>=3000)
