  ...
  //*** Buy  1  ***************************************************************************************** 
       ...
       if (k1_05_buffer[1]-k1_05_buffer[2]>=3 && k2_05_buffer[1]-k2_05_buffer[2]>=0 &&
           ...
           if (((ma05_15_buffer[1]-ma05_15_buffer[2]<=-0.0001  &&  ma08_15_buffer[1]-ma08_15_buffer[1]<=-0.0001 &&
              ...
              ((ma05_30_buffer[1]-ma05_30_buffer[2]<=-0.0001  &&  ma08_30_buffer[1]-ma08_30_buffer[1]<=-0.0001 &&
               ma12_30_buffer[1]-ma12_30_buffer[1]<=-0.0001) ||
              (k1_30_buffer[1]-k1_30_buffer[2]<=-1 && k2_30_buffer[1]-k2_30_buffer[2]<=-1 && k3_30_buffer[1]-k3_30_buffer[2]<=-1 )))
              
               return;
               else  
        
          OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-15*Point,Ask+80*Point,"My order #B1",8801,0,Red );
           ...
           if (((ma05_15_buffer[1]-ma05_15_buffer[2]>=0.0001  &&  ma08_15_buffer[1]-ma08_15_buffer[1]>=0.0001 &&
              ...
              (k1_30_buffer[1]-k1_30_buffer[2]>=1 && k2_30_buffer[1]-k2_30_buffer[2]>=1 && k3_30_buffer[1]-k3_30_buffer[2]>=1)))
               ...
               return;
                        ...
                        OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+15*Point,Bid-80*Point,"My order #S1",4401,0,Green );
     } 
 //***  Ordermodify   ********************************************************************************** 
        ...
        for (cnt=0;cnt<OrdersTotal();cnt++)
          {
             OrderSelect(cnt,SELECT_BY_POS,MODE_TRADES);
              ...
              if (OrderType()==OP_SELL && close_05_buffer[1]-OrderOpenPrice()<=-0.0015) 
                                                                                 //狦OrderTakeProfit()竚块Τぇ莉翴(Bid-80*Point)
                                                                                 //玥盢ぃ穦┪莉,氨穕ゎ
                 OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()-5*Point,OrderTakeProfit(),0,Blue);
             
              if (OrderType()==OP_BUY && close_05_buffer[1]-OrderOpenPrice()>=0.0015) 
                 OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()+5*Point,OrderTakeProfit(),0,Blue);
              
              if (OrderType()==OP_SELL && close_05_buffer[1]-OrderOpenPrice()<=-0.0035) 
                 OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()-20*Point,OrderTakeProfit(),0,Blue);
             
              if (OrderType()==OP_BUY && close_05_buffer[1]-OrderOpenPrice()>=0.0035) 
                 OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()+20*Point,OrderTakeProfit(),0,Blue);

              if (OrderType()==OP_SELL && close_05_buffer[1]-OrderOpenPrice()<=-0.0060) 
                OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()-50*Point,OrderTakeProfit(),0,Blue);
             
              if (OrderType()==OP_BUY && close_05_buffer[1]-OrderOpenPrice()>=0.0060) 
                 OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()+50*Point,OrderTakeProfit(),0,Blue);
