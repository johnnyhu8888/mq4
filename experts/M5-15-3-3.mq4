         ...
         ((close_05_buffer[1]-close_05_buffer[2]<=-0.00010 &&  close_05_buffer[1]-close_05_buffer[3]<=-0.00010 &&  close_05_buffer[1]-close_05_buffer[4]<=-0.00010 ) ||
          ...
          if ((k1_15_buffer[0]-k1_15_buffer[1]>=3 &&  k2_15_buffer[0]-k2_15_buffer[1]>=1 && k3_15_buffer[0]-k3_15_buffer[1]>=0) ||
           ...
           shortsell01=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-250*Point,"My order #S1",4401,0,Green);
                                                                        ...
                                                                        //秨虫30だぃ笆
      if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.01000 &&  k1_240_buffer[1]-k1_240_buffer[2]>=3 &&  k2_240_buffer[1]-k2_240_buffer[2]>=1 && k3_240_buffer[1]-k3_240_buffer[2]>=0)
            OrderModify(OrderTicket(),Ask,OrderOpenPrice()+1000*Point,OrderOpenPrice()+1800*Point,0,Blue);
         else
            ...
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00200 && k1_15_buffer[1]-k1_15_buffer[2]>=3 &&  k2_15_buffer[1]-k2_15_buffer[2]>=1 && k3_15_buffer[1]-k3_15_buffer[2]>=0)
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+100*Point,OrderOpenPrice()+450*Point,0,Blue);
         else
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00010) // && TimeCurrent()-OrderOpenTime()>=2000)
                             //stoploss=OrderOpenPrice()+40*Point寥4翴
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+40*Point,OrderTakeProfit(),0,Blue);
          
      if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.01000 &&  k1_240_buffer[1]-k1_240_buffer[2]<=-3 &&  k2_240_buffer[1]-k2_240_buffer[2]<=-1 && k3_240_buffer[1]-k3_240_buffer[2]<=0)
            ...
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00200 &&  k1_15_buffer[1]-k1_15_buffer[2]<=-3 &&  k2_15_buffer[1]-k2_15_buffer[2]<=-1 && k3_15_buffer[1]-k3_15_buffer[2]<=0)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-100*Point,OrderOpenPrice()-450*Point,0,Blue);
         else
            if (OrderType()==OP_SELL && low_05_buffer[0]-OrderOpenPrice()<=-0.00010) // && TimeCurrent()-OrderOpenTime()>=2000)
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()-40*Point,OrderTakeProfit(),0,Blue);
