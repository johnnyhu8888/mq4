...
//     {              
 ...
 //┮Τ–眎虫冈灿orderticket北恨ぃ璶Τ虫猵祇ネ                   
                     (OrdersTotal()==1 && (longbuy01==OrderTicket() || longbuy02==OrderTicket() || longbuy03==OrderTicket() || longbuy04==OrderTicket() ||
                      ...
                      longbuy01=OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+800*Point,"My order #LB1",8801,0,Yellow );
                      longbuy02=OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+800*Point,"My order #LB2",8802,0,Yellow );
...
//*** long Buy  2  ***************************************************************************************** 
 ...
 //┮Τ–眎虫冈灿orderticket北恨ぃ璶Τ虫猵祇ネ                   
                 if  (OrdersTotal()==1 && (longbuy01==OrderTicket() || longbuy02==OrderTicket() || longbuy03==OrderTicket() || longbuy04==OrderTicket() ||
                      ...
                      longbuy03=OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+1600*Point,"My order #LB3",8803,0,Yellow );
                      longbuy04=OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+1600*Point,"My order #LB4",8804,0,Yellow );
     }  
...
//┮Τ–眎虫冈灿orderticket北恨ぃ璶Τ虫猵祇ネ                   
                     (OrdersTotal()==1 && (longbuy01==OrderTicket() || longbuy02==OrderTicket() || longbuy03==OrderTicket() || longbuy04==OrderTicket() ||
                      ...
                      longsell01=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-800*Point,"My order #LS1",4401,0,Green);
                      longsell02=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-800*Point,"My order #LS2",4402,0,Green);
     }
 ...
 //┮Τ–眎虫冈灿orderticket北恨ぃ璶Τ虫猵祇ネ                   
                 if  (OrdersTotal()==1 && (longbuy01==OrderTicket() || longbuy02==OrderTicket() || longbuy03==OrderTicket() || longbuy04==OrderTicket() ||
                      ...
                      longsell03=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-1600*Point,"My order #LS3",4403,0,Green);
                      longsell04=OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-1600*Point,"My order #LS4",4404,0,Green);
     }      
   ...
   for (cnt=0;cnt<OrdersTotal();cnt++)
     {
       OrderSelect(cnt,SELECT_BY_POS,MODE_TRADES);
                              ...
                              //秨虫15だず⊿害挡虫
         if   (OrderType()==OP_BUY && TimeCurrent()-OrderOpenTime()<=600 &&
                                      ...
                                      shortbuy01==OrderTicket()) 
//                  OrderClose(OrderTicket(),lots,Bid,1,OrangeRed);    
//                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()-50*Point,OrderOpenPrice()+50*Point,0,Blue);
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+160*Point,OrderTakeProfit(),0,Blue);
        
         if (OrderType()==OP_BUY &&  (longbuy01==OrderTicket() || longbuy02==OrderTicket()))
         ...
         if (OrderType()==OP_BUY &&  (longbuy03==OrderTicket() || longbuy04==OrderTicket()))
            ...
            if (OrderType()==OP_BUY && high_05_buffer[0]-OrderOpenPrice()>=0.00120 && high_05_buffer[0]-OrderOpenPrice()<=0.00130) // && TimeCurrent()-OrderOpenTime()>=2000)
                  ...
                  OrderModify(OrderTicket(),Ask,OrderOpenPrice()+60*Point,OrderTakeProfit(),0,Blue);
                            ...
                            //秨虫15だず⊿害挡虫
         if   (OrderType()==OP_SELL && OrderOpenPrice()-low_05_buffer[0]>=-0.00060 && OrderOpenPrice()-low_05_buffer[1]>=-0.00060 &&
                                       OrderOpenPrice()-low_05_buffer[2]>=-0.00060 && OrderOpenPrice()-low_05_buffer[3]>=-0.00060 &&
                                       shortsell01==OrderTicket()) 
                  OrderModify(OrderTicket(),Bid,OrderOpenPrice()+50*Point,OrderOpenPrice()-50*Point,0,Blue);
        
         if (OrderType()==OP_SELL &&  (longsell01==OrderTicket() || longsell02==OrderTicket()))
         ...
         if (OrderType()==OP_SELL &&  (longsell03==OrderTicket() || longsell04==OrderTicket()))
