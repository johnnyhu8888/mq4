...
//           if    (OrderType()==OP_BUY && (OrderMagicNumber( )==8807 || OrderMagicNumber( )==8806) &&
 ...
 //當反向時確認不在往回調整
                 k1_15_buffer[1]-k1_15_buffer[2]>=1 &&                
                 high_05_buffer[0]-OrderOpenPrice()>=0.00550 && high_05_buffer[0]-OrderOpenPrice()<=0.00560) 
...
//           if    (OrderType()==OP_SELL && (OrderMagicNumber( )==4401 || OrderMagicNumber( )==44011) &&
 ...
 //當反向時確認不在往回調整
                  k1_15_buffer[1]-k1_15_buffer[2]<=-1 &&                
                  low_05_buffer[0]-OrderOpenPrice()<=-0.00550 && low_05_buffer[0]-OrderOpenPrice()>=-0.00560) 
 ...
 //當反向時確認不在往回調整
                  k1_15_buffer[1]-k1_15_buffer[2]<=-1 &&                
                  low_05_buffer[0]-OrderOpenPrice()<=-0.00350 && low_05_buffer[0]-OrderOpenPrice()>=-0.00360) 
...
//H1,H4 kd1,2,3值差全部>1
 ...
 //H4  K1,K2,K3全上揚且[1]為正 
                   (k1_240_buffer[1]-k1_240_buffer[2]>0 && k2_240_buffer[1]-k2_240_buffer[2]>0 && 
                    k3_240_buffer[1]-k3_240_buffer[2]>0 && close_240_buffer[1]-open_240_buffer[1]>=0.00030) ||
...
//    if ( OrdersTotal()>0 && OrderMagicNumber() == 4410) 
 ...
 //m240之ma05與ma12差距大過40點
                    ma05_240_buffer[1]-ma12_240_buffer[1]>=0.00400 ||
                     
 //m240 k1,k2,k3皆上升,且K2,K3在45下
                   (k1_240_buffer[1]-k1_240_buffer[2]>=0 && k2_240_buffer[1]-k2_240_buffer[2]>=0 &&
                    k3_240_buffer[1]-k3_240_buffer[2]>=0 && k2_240_buffer[1]<=45 && k3_240_buffer[1]<=45)) 
                    return;
                    else
                    OrderClose(OrderTicket(),lots,Bid,1,LimeGreen); 
...
//    if ( OrdersTotal()>0 && OrderMagicNumber() == 8810) 
 ...
 //m240 k1,k2,k3皆下降,且K2,K3在55上
                    (k1_240_buffer[1]-k1_240_buffer[2]<=0 && k2_240_buffer[1]-k2_240_buffer[2]<=0 &&
                     k3_240_buffer[1]-k3_240_buffer[2]<=0 && k2_240_buffer[1]>=55 && k3_240_buffer[1]>=55)) 
                     return;
                     else      
                     OrderClose(OrderTicket(),lots,Ask,1,Gold);    
...
//m240  K2,K3全下降
 ...
 //m15 [1]收黑 [0]小於[1][2][3]
           (close_15_buffer[1]-open_15_buffer[1]<=-0.00010 && close_15_buffer[0]-close_15_buffer[1]<=-0.00010 &&
            close_15_buffer[0]-close_15_buffer[2]<=-0.00010 && close_15_buffer[0]-close_15_buffer[3]<=-0.00010 &&
            k1_15_buffer[1]>=50 && k2_15_buffer[1]>=50 && k3_15_buffer[1]>=50)))
            OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+300*Point,Bid-1900*Point,"My order #S2",4402,0,Green);
