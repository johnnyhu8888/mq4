...
//trade=1;
 ...
 //**************************   oders modify  ***************************************************************88
           ...
           if    (OrderType()==OP_BUY && (OrderMagicNumber( )==8801 || OrderMagicNumber( )==88011) &&
                  high_05_buffer[0]-OrderOpenPrice()>=0.00400 && high_05_buffer[0]-OrderOpenPrice()<=0.00410) 
                             //stoploss=OrderOpenPrice()+40*Point寥4翴
...
//**********************************************************************************************************
           ...
           if    (OrderType()==OP_SELL && (OrderMagicNumber( )==4401 || OrderMagicNumber( )==44011) &&
                  low_05_buffer[0]-OrderOpenPrice()<=-0.00400 && low_05_buffer[0]-OrderOpenPrice()>=-0.00410) 
...
//**************************   oders close  ***************************************************************88
//close虫狦禦芥岿竚盢穦掸close 秨虫碞close 
//**************************   oders close  ***************************************************************88
...
//セ琿Τノ 
            ...
            if    (OrderType()==OP_BUY && OrderMagicNumber( )==8801) // || OrderMagicNumber( )==88011))
                                //虫,1.5
              if  (TimeCurrent()- OrderOpenTime()>=5000 && TimeCurrent()- OrderOpenTime()<=7000 &&
...
if  (OrdersTotal()<2) 
     if  ( OrdersTotal()>0 && OrderMagicNumber()==44011) 
       ...
       OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-19000*Point,"My order #LS11",44011,0,Blue);
...
if  (OrdersTotal()<2) 
    if ( OrdersTotal()>0 && OrderMagicNumber() == 88011) 
      ...
      OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+19000*Point,"My order #LB11",88011,0,Yellow );
           ...
           (k1_30_buffer[0]-k1_30_buffer[1]>=3 &&  k2_30_buffer[0]-k2_30_buffer[1]>=1 && k3_30_buffer[0]-k3_30_buffer[1]>=0 &&
            ...
            if   ((d2_60_buffer[1]>=45 && d3_60_buffer[1]>=45 &&
                 ...
                 ((k1_240_buffer[1]-d1_240_buffer[1])+(k2_240_buffer[1]-d2_240_buffer[1])+(k3_240_buffer[1]-d3_240_buffer[1]))<50) ||
                  ...
                  OrderSend(Symbol(),OP_BUY,lots,Ask,1,Ask-150*Point,Ask+19000*Point,"My order #LB1",8801,0,Yellow );
//*** long Buy  2  ***************************************************************************************** 
   if ( OrdersTotal()>0 && OrderMagicNumber() == 8806 ) 
      return;
      else
          ...
          if  (k1_05_buffer[1]-d1_05_buffer[1]<=-3 &&   k1_05_buffer[1]>=50)                               
               ...
               if   ((d2_30_buffer[1]<=55 && d3_30_buffer[1]<=55 &&
                  ...
                  (macdsignalline_240_buffer[1]-macdsignalline_240_buffer[2]<=-0.000050) && (macdsignalline_240_buffer[2]-macdsignalline_240_buffer[3]>=0.000050)))
                   ...
                   OrderSend(Symbol(),OP_SELL,lots,Bid,1,Bid+150*Point,Bid-19000*Point,"My order #LS1",4401,0,Blue);
